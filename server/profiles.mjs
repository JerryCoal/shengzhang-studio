...
export function createProfileApp(options) {
  ...
  app.get('/api/health', (_req, res) => res.json({ ok: true, authRequired: true, authMode: 'profiles', version: '1.0.0', localOnly: true }));
  ...
  function openWorkspace(user, masterKey) {
    ...
    try {
      ...
      const workspaceApp = createApp(store, { ...options.workspaceOptions, profileMode: true, staticDirectory, allowedOrigins, vault: vault('openai'), deepseekVault: vault('deepseek'), seedanceVault: vault('seedance'), seedreamVault: vault('seedream'), douyinVault: vault('douyin'), modelStore: createModelStore(resolve(directory, 'models.json')), integrationPreferences: createIntegrationPreferences(resolve(directory, 'integrations.json')) });
