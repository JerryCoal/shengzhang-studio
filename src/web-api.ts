...
import { defaultSeedream, seedreamConnectionSchema } from '../server/seedream-config.mjs';
import type { ImageProvider } from './types';
import { providerLabel } from './model-provider';
import { resolveWebBackend, requestWebOperation, webCallbackUrl } from './web-connection';

const defaults = (): WebPrivate => ({ credentials: {}, seedream: { ...defaultSeedream }, imageProvider: 'openai', routes: { ...catalog.routes }, seedance: { region: 'volcengine', model: '', reservationUsd: 2, outputPriceUsd: 0 }, authorizations: [], verification: null });
...
function integrationsOf(state: State): Integrations {
  ...
  return { local: true, imageProvider: saved.imageProvider || 'openai', seedream: { ...(saved.seedream || defaultSeedream), supported: true, configured: !!saved.credentials.seedream, suffix: saved.credentials.seedream?.slice(-4) || '', problem: '' }, seedance: { ...saved.seedance, supported: true, configured: !!saved.credentials.seedance, suffix: saved.credentials.seedance?.slice(-4) || '', problem: '' },
...
function textModel(saved: WebPrivate, path: string, body: unknown): string {
  if (path.endsWith('/cover')) return (saved.seedream || defaultSeedream).model;
  if (path.endsWith('/keyframes')) return ((body as { provider?: ImageProvider })?.provider || saved.imageProvider) === 'seedream' ? (saved.seedream || defaultSeedream).model : 'gpt-image-2';
...
function requestProvider(saved: WebPrivate, path: string, body: unknown): TextProvider | ImageProvider | undefined {
  if (path.endsWith('/cover')) return 'seedream';
  if (path.endsWith('/keyframes')) return (body as { provider?: ImageProvider })?.provider || saved.imageProvider || 'openai';
  if (path === '/settings/credential/check') return 'openai';
...
async function relay(path: string, method: string, body: unknown, projectId?: string) {
  return exclusive(async () => {
    const endpoint = await resolveWebBackend();
    const started = await changeLocalState(state => {
      ...
      const provider = requestProvider(state.webPrivate, path, body), limited = provider === 'deepseek' ? state.webPrivate.deepseekDiagnostic : provider === 'openai' ? state.webPrivate.apiDiagnostic : null;
      if (limited?.kind === 'rate_limit' && limited.model === textModel(state.webPrivate, path, body) && limited.retryAt && Date.parse(limited.retryAt) > Date.now()) throw new Error(`${providerLabel(provider === 'deepseek' ? 'deepseek' : 'openai')} 仍在限流等待期，请 ${Math.ceil((Date.parse(limited.retryAt) - Date.now()) / 1000)} 秒后再试。本次未发送新请求。`);
    ...
    if (provider === 'seedream' && saved.credentials.seedream) credentials.seedream = saved.credentials.seedream;
    ...
    const envelope = { path, method, body, state: base, credentials, routes: saved.routes, seedance: saved.seedance, seedream: saved.seedream || defaultSeedream, imageProvider: saved.imageProvider || 'openai', authorizations: douyin ? saved.authorizations : [] };
    let response;
    try {
      const encoded = JSON.stringify(envelope);
      if (new TextEncoder().encode(encoded).byteLength > 32 * 1024 * 1024) throw new Error('此项目的素材超过单次联网请求 32 MB 上限，请减少旧成品素材后再试');
      const request = await requestWebOperation(endpoint, encoded);
...
export async function webAPI<T>(path: string, method = 'GET', body?: unknown): Promise<T> {
  if (/^\/projects\/[^/]+\/trends\/refresh$/.test(path) && method === 'POST') return await relay(path, method, body, path.split('/')[2]) as T;
  ...
  if (path === '/integrations/image-provider' && method === 'PUT') {
    const provider = z.object({ provider: z.enum(['openai', 'seedream']) }).strict().parse(body).provider;
    await saveConnection(saved => { saved.imageProvider = provider; }); return integrationsOf(await readLocalState()) as T;
  }
  if (path === '/integrations/seedream' && ['PUT', 'DELETE'].includes(method)) {
    await saveConnection(saved => {
      if (method === 'DELETE') { delete saved.credentials.seedream; return; }
      const { apiKey, settings } = seedreamConnectionSchema.parse(body);
      saved.seedream = settings; if (apiKey) saved.credentials.seedream = apiKey;
  ...
  if (path === '/integrations/douyin' && ['PUT', 'DELETE'].includes(method)) {
    await saveConnection((saved, state) => {
      if (method === 'DELETE') delete saved.credentials.douyin;
        ...
        if (url.href !== webCallbackUrl()) throw new Error(`请在平台登记并填写 ${webCallbackUrl()}`);
