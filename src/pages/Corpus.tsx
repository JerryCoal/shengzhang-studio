...
export function CorpusPage() {
  ...
  return <>
    ...
    <GenerationWorkflow/>
    <section className="panel corpus-search"><div className="section-heading"><h2><Search size={18}/>查询语料库</h2><span>{documents.filter(d => d.enabled).length} 份参与检索</span></div><form onSubmit={e => { e.preventDefault(); void run(async () => { const value = await api<{ results: CorpusReference[] }>(`/projects/${project.id}/corpus/search`, 'POST', { query }); setResults(value.results); }); }}><Field label="想查找什么"><input value={query} maxLength={500} placeholder="例如：产品规格、使用步骤、品牌定位" onChange={e => setQuery(e.target.value)}/></Field><InputAssist kind="query" value={query} onChange={v => { setQuery(v); setResults(null); }} maxLength={500}/><button className="button primary" disabled={busy || !query.trim()}><Search size={16}/>查询相关段落</button></form><p className="small-note">本地按关键词匹配，最多返回 5 段。只检索当前项目已启用的文档，不调用外部模型。</p>{results && (results.length ? <RetrievalResults results={results}/> : <p className="empty-search">没有找到相关段落，请换一个关键词或补充文档。</p>)}</section>
