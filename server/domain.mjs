import { z } from 'zod';
import { strategyPrompt, taskPrompt } from './prompts.mjs';
...
export function tasksFor(brief, strategy, rules = defaultRules()) {
  return brief.channels.flatMap(channel => [reviewObject({
    ...
    prompt: taskPrompt(brief, strategy, channel),
...
export function updateAsset(p, assetId, input, rules = defaultRules()) {
  ...
  const data = z.object({ title: text(200).min(1), body: text(), mediaData: z.string().max(24000000).optional(), mime: z.enum(['image/png', 'image/jpeg', 'video/webm', 'video/mp4']).optional(), expectedRevision: z.number().int() }).parse(input);
  ...
  if (data.mediaData) assert(/^data:(image\/(?:png|jpeg)|video\/webm|video\/mp4);base64,[A-Za-z0-9+/=]+$/.test(data.mediaData), '素材格式无效');
  if (data.mediaData) {
    assert(data.mediaData.startsWith(`data:${data.mime};base64,`), '素材类型不一致');
    assert(asset.kind === 'image' ? ['image/png', 'image/jpeg'].includes(data.mime) : data.mime.startsWith('video/'), '素材与任务类型不一致');
    ...
    assert(data.mime === 'image/png' ? hex(bytes.slice(0, 8)) === '89504e470d0a1a0a' : data.mime === 'image/jpeg' ? hex(bytes.slice(0, 3)) === 'ffd8ff' : data.mime === 'video/webm' ? hex(bytes.slice(0, 4)) === '1a45dfa3' : bytes.slice(4, 8) === 'ftyp', '文件内容与格式不一致');
  }
