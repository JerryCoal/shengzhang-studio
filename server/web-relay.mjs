...
import { createWebJobs } from './web-jobs.mjs';
...
const envelopeSchema = z.object({
  ...
  state: stateSchema,
  credentials: z.object({ openai: z.string().max(512).optional(), deepseek: z.string().max(512).optional(), seedance: z.string().max(512).optional(), seedream: z.string().max(512).optional(), douyin: z.string().max(100000).optional() }).strict(),
  routes: routesSchema,
  seedance: z.unknown().optional(), seedream: z.unknown().optional(), imageProvider: z.enum(['openai', 'seedream']).optional(),
...
export function createWebRelay({ staticDirectory = resolve('web-dist'), publicOrigin, frontendOrigins = [], allowLocalHttp = false, workspaceOptions = {}, maxActive = 1 } = {}) {
  ...
  const allowedOrigins = [origin.origin, ...frontendOrigins.map(value => { const parsed = new URL(value); if (parsed.protocol !== 'https:' || parsed.href !== parsed.origin + '/') throw new Error('网页来源必须是 HTTPS 根地址'); return parsed.origin; })];
  const app = express(); app.disable('x-powered-by');
  const jobs = createWebJobs({ host: origin.host, publicOrigin: origin.origin });
  ...
  app.post('/api/web/jobs', (req, res, next) => {
    if (req.get('X-Studio-Client') !== 'studio-v1' || !req.is('application/json')) return res.status(403).json({ error: '请从工作区发起请求' });
    if (jobs.size() >= 4 || active >= maxActive) return res.status(429).json({ error: '联网服务繁忙，请稍后重试；尚未调用服务商' });
    next();
  }, express.json({ limit: '32mb' }), (req, res) => {
    const parsed = envelopeSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: '请求格式或数据大小不符合要求，未调用服务商' });
    if (!allowedWebOperation(parsed.data.path, parsed.data.method)) return res.status(404).json({ error: '不支持此联网操作，未调用服务商' });
    const job = jobs.start(req); req.body = undefined;
    return job ? res.status(202).json({ job }) : res.status(429).json({ error: '联网服务繁忙，尚未调用服务商' });
  });
  app.all('/api/web/jobs', (req, res) => {
    if (!['GET', 'DELETE'].includes(req.method) || req.get('X-Studio-Client') !== 'studio-v1') return res.sendStatus(403);
    const id = req.get('Authorization')?.replace(/^Bearer /, ''), job = jobs.read(id);
    if (!job) return res.status(404).json({ error: '请求结果不存在或已过期，请核对服务商记录，勿重复提交' });
    if (req.method === 'DELETE') { jobs.remove(id); return res.json({ ok: true }); }
    return job.done ? res.status(job.status).json(job.value) : res.status(202).json({ pending: true });
  });
  app.post('/api/web/execute', (req, res, next) => {
    ...
    try {
      ...
      if (input.seedream) preferences.saveSeedream(input.seedream);
      if (input.imageProvider) preferences.saveImageProvider(input.imageProvider);
      for (const [key, value] of input.authorizations) if (value.expiresAt > Date.now()) authorizations.set(key, value);
      workspace = createApp(store, { ...workspaceOptions, apiKey: '', password: '', allowedOrigins,
        authorizeCredentials: request => request === req, modelStore, integrationPreferences: preferences, authorizations,
        vault: memoryVault(input.credentials, 'openai'), deepseekVault: memoryVault(input.credentials, 'deepseek'), seedanceVault: memoryVault(input.credentials, 'seedance'), seedreamVault: memoryVault(input.credentials, 'seedream'), douyinVault: memoryVault(input.credentials, 'douyin'),
      });
  ...
  app.locals.close = async () => { closing = true; await Promise.allSettled([...running]); rates.clear(); jobs.close(); };
  return app;
}
